...
class WhatsAppParser:
    ...
    def parse_file(self, file_path: str) -> List[Dict]:
        ...
        try:
            ...
            # Streaming read with generator to save memory
            for encoding in encodings:
                try:
                    with open(file_path, 'r', encoding=encoding) as f:
                        # Test reading first line to verify encoding
                        f.readline()
                        f.seek(0)
                        
                        logger.info(f"Reading file with {encoding} encoding (stream mode)")
                        ...
                        for line in f:
                            ...
                            if match:
                                ...
                                # Detect message type
                                msg_type = self._detect_message_type(body)
                                
                                # If media attached, convert to ID reference
                                media_id = None
                                if msg_type == 'media_attached':
                                    body, media_id = self._convert_media_to_id(body)
                                ...
                                if media_id:
                                    current_message['media_id'] = media_id
                            else:
                        ...
                        # If we reached here, reading was successful
                        logger.info(f"{lines_read} lines processed, {len(self.messages)} messages parsed")
            ...
            logger.error(f"File could not be read with any supported encoding")
            return []
    ...
    def _detect_message_type(self, body: str) -> str:
        ...
        Returns:
            Message type (text, image, video, audio, document, link, media_attached)
        """
        ...
        if '<medya dahil edilmedi>' in body_lower or '<media omitted>' in body_lower:
            ...
            return 'link'
        elif '(dosya ekli)' in body_lower or '(file attached)' in body_lower:
    ...
    def _convert_media_to_id(self, body: str) -> tuple[str, str | None]:
        """
        Convert media filename in message body to media ID reference.
        
        Patterns:
        - "‎IMG-20260103-WA0014.jpg (dosya ekli)" -> "[IMG-20260103-WA0014]"
        - "‎sindirim sistemi özet.pptx (dosya ekli)" -> "[PPTX-sindirim_sistemi_ozet]"
        
        Args:
            body: Original message body
            
        Returns:
            Tuple of (modified body, media_id or None)
        """
        # Pattern for attached files: filename (dosya ekli) or filename (file attached)
        pattern = r'[\u200e\u200f]?(.+?)\s*\((?:dosya ekli|file attached)\)'
        match = re.search(pattern, body, re.IGNORECASE)
        
        if match:
            filename = match.group(1).strip()
            # Generate media ID from filename
            media_id = self._generate_media_id(filename)
            # Replace the pattern with media ID reference
            new_body = f"[{media_id}]"
            return new_body, media_id
        
        return body, None
    
    def _generate_media_id(self, filename: str) -> str:
        """
        Generate media ID from filename (same logic as MediaManager).
        ...
        # For other files, create prefixed ID
        ext_to_prefix = {
            '.pdf': 'PDF',
            '.pptx': 'PPTX',
            '.ppt': 'PPTX',
        ...
        prefix = ext_to_prefix.get(ext, 'FILE')
