...
class ContextTools:
    ...
    def _parse_list_param(self, value, param_name: str = "param") -> list:
        """Parse a parameter that should be a list but might come as string.
        ...
        if isinstance(value, list):
            # Check if it's a list with a single string that looks like a list
            if len(value) == 1 and isinstance(value[0], str) and value[0].startswith('['):
                try:
                    import ast
                    return ast.literal_eval(value[0])
                except:
                    pass
            # Check each item - if any item looks like a stringified list, parse it
            result = []
            for item in value:
    ...
    def create_context(
        ...
        try:
            ...
            # Parse list parameters using helper function
            parsed_tags = self._parse_list_param(tags, "tags")
            parsed_attendees = self._parse_list_param(attendees, "attendees")
            parsed_related = self._parse_list_param(related_context_ids, "related_context_ids")
    ...
    def delete_context(self, context_id: str) -> str:
        ...
        This will also remove all links to this context from other contexts.
