...
class L2_5:
    ...
    def _ensure_data_file(self) -> None:
        ...
        if not os.path.exists(self.data_file):
            ...
            try:
                ...
                print(f"Error normalizing L2.5.json: {e}")
                # Recreate file
                initial_data = {
                    "search_index": {
                        "summaries": [],
    ...
    def _normalize_data(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if "summaries" not in search_index or not isinstance(search_index["summaries"], list):
            search_index["summaries"] = []
        if "total_summaries" not in search_index or not isinstance(search_index["total_summaries"], int):
            search_index["total_summaries"] = len(search_index["summaries"])
        if "summary_index" not in search_index or not isinstance(search_index["summary_index"], dict):
            search_index["summary_index"] = {}
        ...
        metadata.setdefault("version", "2.5")
