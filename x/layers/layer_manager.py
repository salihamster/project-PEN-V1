...
class LayerManager:
    ...
    def add_user_message(self, content: str, metadata: dict = None) -> bool:
        """
        Add a user message to L1.
        
        Args:
            content: User message content
            metadata: Optional metadata (e.g., attached files info)
            
        Returns:
            True if successful
        """
        try:
            self.l1.add_message(MessageRole.USER, content, metadata=metadata)
            return True
    ...
    def search_memory(
        ...
        """
        Search historical sessions using L2.5 (keywords) and L2 (dates).
        """
        results = []
        seen_ids = set()

        # 1. Date-based search (L2)
        if start_date:
            try:
                s_date = datetime.strptime(start_date, "%Y-%m-%d")
                e_date = datetime.strptime(end_date, "%Y-%m-%d") if end_date else datetime.now()
                # Use L2 search
                date_results = self.l2.search_sessions_by_date_range(s_date, e_date)
                
                # Fetch summaries for these sessions from L2.5
                for session in date_results:
                    sid = session.get("session_id")
                    if sid and sid not in seen_ids:
                        summary_data = self.l2_5.get_summary_by_session_id(sid)
                        if summary_data:
                            results.append({
                                "session_id": sid,
                                ...
                                "relevance_score": 10  # High score for date match
                            })
                            seen_ids.add(sid)
            except Exception as e:
                print(f"Date search error: {e}")

        # 2. Keyword-based search (L2.5) - Only if we need more results or no date provided
        if len(results) < max_results:
            try:
                keyword_results = self.l2_5.search_by_text(query, max_results)
                for res in keyword_results:
                    sid = res.get("session_id")
                    if sid and sid not in seen_ids:
                        summary_data = res.get("summary_data", {})
                        results.append({
                            "session_id": sid,
                            "summary": summary_data.get("summary", ""),
                            "keywords": summary_data.get("keywords", []),
        ...
        return sorted(results, key=lambda x: (x.get("relevance_score", 0), x.get("created_at", "")), reverse=True)[:max_results]
