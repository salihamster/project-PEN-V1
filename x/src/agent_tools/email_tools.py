...
class EmailTools:
    ...
    def _simplify_emails(self, emails: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Convert full email objects to simplified versions for list/search views.
        Excludes heavy fields like body_full and html_body_full.
        """
        simplified = []
        for e in emails:
            # Use existing summary fields or truncate full content
            body_preview = e.get('body', '') or e.get('text', '') or ''
    ...
    def search_by_sender(
        ...
        try:
            ...
            result = {
                ...
                "emails": self._simplify_emails(filtered)
            }
    ...
    def get_email_thread(
        ...
        try:
            ...
            result = {
                ...
                "emails": self._simplify_emails(thread_emails)
            }
    ...
    def get_email_content(
        ...
        try:
            ...
            # Use FULL content versions for get_email_content (filtered HTML but no hard limits)
            # Fall back to summary versions if full versions don't exist (backward compatibility)
            body_content = target.get("body_full") or target.get("body") or target.get("text") or ""
            html_content = target.get("html_body_full") or target.get("html_body") or target.get("html") or ""
            ...
            resp = {
                ...
                "email": {
                    ...
                    "body": body_content,  # Full text content (no limit)
                    "html_body": html_content,  # Filtered HTML (no hard limit, but cleaned)
